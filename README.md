...
**Кассир** — принимает заказы, управляет меню, открывает отчёт за смену.

**Бариста** — видит очередь заказов (Новые / В работе / Готово), меняет статусы.
...
3. Render автоматически найдёт `Dockerfile` и использует его для сборки
...
2. Нажать **Управление меню** → добавить напитки и десерты
3. Принимать заказы через форму
4. Бариста на своём устройстве открывает роль **Бариста** и обновляет статусы
5. По окончании смены кассир открывает **Отчёт за смену** → **Предпросмотр чека** → скачивает PNG
